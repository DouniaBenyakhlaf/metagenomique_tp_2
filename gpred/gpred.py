...
def find_start(start_regex: Pattern, sequence: str, start: int, stop: int) -> Union[int, None]:
    ...
    """
    first_occ = start_regex.search(sequence, start, stop)
    if first_occ:
        return first_occ.start(0)
    else:
        return None
...
def find_stop(stop_regex: Pattern, sequence: str, start: int) -> Union[int, None]:
    ...
    """
    iter_stop = stop_regex.finditer(sequence, start)
    for stop in iter_stop:
...
def has_shine_dalgarno(shine_regex: Pattern, sequence: str, start: int, max_shine_dalgarno_distance: int) -> bool:
    ...
    """
    find_dalgarno = shine_regex.search(sequence, start-max_shine_dalgarno_distance, start-6)
    if(find_dalgarno and find_dalgarno.start(0)>0):
        return True
    return False
...
def predict_genes(sequence: str, start_regex: Pattern, stop_regex: Pattern, shine_regex: Pattern, 
    ...
    while len_seq - current_pos >= min_gap:
        current_pos = find_start(start_regex, sequence, current_pos, len_seq)
